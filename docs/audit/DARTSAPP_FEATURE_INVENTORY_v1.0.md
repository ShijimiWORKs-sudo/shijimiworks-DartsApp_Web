...
MATCHはPR #7時点で `KEEP` に分類済みだったため、mainマージ後も分類件数は変わらない。状態のみ `IMPLEMENTED_IN_PR_PENDING_MERGE` から `IMPLEMENTED` へ更新する。
...
| INV-009 | MATCH DB foundation                        | `matches`, `match_players`, `match_player_results`                             | DB基盤と一意制約あり                            | KEEP            | DartsApp正式MATCHの土台                                               | migration 001                | 維持                                     |
| INV-010 | MATCH UI / service                         | `app/game/match/**`, `MatchGameService`, `domain/match/**`                     | IMPLEMENTED                                     | KEEP            | PR #7 Phase 6 / main merge `5c5cfa17e28987cdb2b5815e09ea5dbaed1dc77c` | MATCH DB / common-contract   | PC Web操作性QA                           |
| INV-011 | Game services layer                        | `features/game/application/services/**`                                        | COUNT-UP/01/CRICKET/MATCH serviceあり           | KEEP            | 画面からSQL直接実行しない構造                                         | SQLite executor              | 維持                                     |
...
| INV-017 | Common Account contract                    | `features/common-contract/**`                                                  | Account/Game/Rating/Event/Outbox/MATCH JSON境界 | SHARED_CONTRACT | 共通化対象はデータ契約                                                | SQLite / validators          | 維持                                     |
...
| INV-039 | Game tests                                 | `tests/game/**`                                                                | COUNT-UP/01/CRICKET/MATCH/DB/Rating tests       | KEEP            | DartsAppゲーム品質に必要                                              | node sqlite adapter          | PC Web smoke追加                         |
...
## Phase 6 MATCH Implementation Confirmation

Phase 6 MATCHはPR #7 `Phase 6: two-player match vertical slice` で実装され、
main merge commit `5c5cfa17e28987cdb2b5815e09ea5dbaed1dc77c` によりmainへ取り込まれた。
実機確認も完了しているため、棚卸し状態は `IMPLEMENTED` と記録する。
