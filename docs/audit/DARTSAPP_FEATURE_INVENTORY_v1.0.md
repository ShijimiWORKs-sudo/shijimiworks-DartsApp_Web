...
| INV-009 | MATCH DB foundation                        | `matches`, `match_players`, `match_player_results`                             | DB基盤と一意制約あり                        | KEEP            | DartsApp正式MATCHの土台                              | migration 001                | Phase 6本体をmainへ統合                  |
| INV-010 | MATCH UI / service                         | spec: `/game/match/**`                                                         | mainには未実装                              | REVIEW          | 画面遷移仕様v1.1には存在、実装差分あり               | MATCH DB                     | P1でPhase 6 PR統合                       |
| INV-011 | Game services layer                        | `features/game/application/services/**`                                        | COUNT-UP/01/CRICKET serviceあり             | KEEP            | 画面からSQL直接実行しない構造                        | SQLite executor              | MATCH/将来練習も同層へ追加               |
...
| INV-017 | Common Account contract                    | `features/common-contract/**`                                                  | Account/Game/Rating/Event/Outbox JSON境界   | SHARED_CONTRACT | 共通化対象はデータ契約                               | SQLite / validators          | MATCH JSONをPhase 6後に追加              |
...
| INV-039 | Game tests                                 | `tests/game/**`                                                                | COUNT-UP/01/CRICKET/DB/Rating tests         | KEEP            | DartsAppゲーム品質に必要                             | node sqlite adapter          | MATCH/PC Web smoke追加                   |
