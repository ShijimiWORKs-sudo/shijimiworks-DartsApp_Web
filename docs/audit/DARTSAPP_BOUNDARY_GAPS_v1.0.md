...
| GAP-004 | MATCH本体                     | mainには `/game/match/**` がない                      | DartsApp主機能不足           | P1       |
...
## 3. DartsAppに混入している可能性があるSupport機能
...
| MATCH JSON                    | mainでは未実装                 | P1 after MATCH  |
...
| SPEC-002 | MATCH routeあり                         | mainに `/game/match/**` なし                | 主機能不足             |
...
| ID     | DB                                                 | 画面/Service                   | 判断               |
| ------ | -------------------------------------------------- | ------------------------------ | ------------------ |
| DB-001 | `matches`, `match_players`, `match_player_results` | mainにMATCH画面・serviceなし   | P1                 |
| DB-002 | `rating_evaluations.source_type = match`           | MATCH本体なし                  | P1/P2              |
